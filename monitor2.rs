...
pub struct MonitorMetrics {
    pub metrics: Metrics,
    pub rpc_health: DashMap<String, bool>,
    pub last_alert_time: Option<TimeWrapper>,
    pub monitor_start_time: TimeWrapper,
}
...
impl RpcPool {
    ...
    pub fn get_healthy_client(&self) -> Option<Arc<DebugRpcClient>> {
...
impl TokenMonitor {
    ...
    async fn new() -> Result<Self> {
        ...
        let monitor_state = Self::load_monitor_state().await?;
    ...
    async fn fetch_token_info(&self, mint: &Pubkey) -> Result<TokenInfo> {
        ...
        Ok(TokenInfo {
            ...
            lock_details: data.lock_details,
    ...
    fn format_message(&self, analysis: &TokenAnalysis) -> String {
        let data = NotificationData {
            ...
            first_trade_time: TimeWrapper(SystemTime::now()),
    ...
    fn test_monitor_output(&self) {
        ...
        let mock_analysis = TokenAnalysis {
            ...
            detection_time: chrono::Local::now().into(),
            first_trade_time: chrono::Local::now().into(),
            liquidity_add_time: chrono::Local::now().into(),
            monitor_id: 0,
            risk_level: 0,
...
"#,
    ...
    fn generate_notification(&self, analysis: &TokenAnalysis) -> NotificationData {
        NotificationData {
            ...
            risk_level: analysis.risk_score.to_string(),
            attention_level: 0.to_string(),
            symbol: analysis.token_info.symbol.clone(),
            name: analysis.token_info.name.clone(),
            contract: analysis.token_info.mint.to_string(),
            creator: analysis.token_info.creator.to_string(),
    ...
    async fn fetch_token_info(&self, mint: &Pubkey) -> Result<TokenInfo> {
        ...
        let data = if !account.data.is_empty() {
            ...
            TokenInfo {
                ...
                lock_details: token_data.lock_details,
                total_inflow: token_data.total_inflow,
            }
        } else {
            return Err(anyhow!("Empty account data"));
        };
    ...
    async fn update_rpc_health(&self, url: String, status: bool) {
        ...
        self.rpc_pool.health_status.insert(url, status);
    }
...
impl Default for NotificationData {
    fn default() -> Self {
        Self {
            ...
            detection_time: TimeWrapper::default(),
            first_trade_time: TimeWrapper::default(),
            liquidity_add_time: TimeWrapper::default(),
        }
    }
}
